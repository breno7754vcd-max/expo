...
it('should convert header items children, correctly to options', () => {
  ...
  expect(leftHeaderItems[2].element.type).toBe(View);
  if (leftHeaderItems[2].element.type !== View) throw new Error('Element type is not View');
  expect(leftHeaderItems[2].element.props.children.type).toBe(CustomHeaderElement);
...
it('omits hidden Stack.Header.View from header items', () => {
  ...
  // The element wraps our child, so inspect its children for testID
  const child = element.props.children;
  expect(child.props.testID).toBe('visible-view');
});
