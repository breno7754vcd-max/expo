...
function getQualifiedRouteComponent(value) {
    ...
    ...props }) {
        ...
        {value.type === 'route' && navigationEvents_1.unstable_navigationEvents.hasAnyListener() && (<AnalyticsListeners navigation={navigation}/>)}
...
function AnalyticsListeners({ navigation }) {
    const stateForPath = (0, native_1.useStateForPath)();
    ...
    (0, react_1.useEffect)(() => {
        if (pathname) {
            const cleanTransitionStart = navigation.addListener('transitionStart', (e) => {
                const type = e.data?.closing ? 'pageWillDisappear' : 'pageWillShow';
                navigationEvents_1.unstable_navigationEvents.emit(type, {
                    pathname,
                });
            });
            const cleanTransitionEnd = navigation.addListener('transitionEnd', (e) => {
                const type = e.data?.closing ? 'pageDidDisappear' : 'pageDidShow';
                navigationEvents_1.unstable_navigationEvents.emit(type, {
                    pathname,
                });
            });
